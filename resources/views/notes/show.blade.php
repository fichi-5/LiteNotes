            ...
            <div class="flex">
                @if(!$note->trashed())
                    ...
                    <form action="{{ route('notes.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure to this note to the trash?');">Move to Trash</button>
                    </form>
                @else
                    ...
                    <form action="{{ route('trashed.destroy', $note) }}" method="post">
                        @method('delete')
                        @csrf
                        <button type="submit" class="btn btn-danger ml-4" onclick="return confirm('Are you sure to delete this note forever? This action cannot be undone.');">Delete Forever</button>
                    </form>
                @endif
            </div>
